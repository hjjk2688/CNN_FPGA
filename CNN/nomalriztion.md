...
양자화가 '실수를 정수로 바꾸는 전략'이라면, **고정 소수점**은 하드웨어가 그 정수를 가장 빠르게 처리하기 위한 **데이터 포맷(규격)**입니다.

### 🔹 왜 고정 소수점 포맷을 쓰는가?
데이터 포맷을 고정 소수점으로 미리 맞춰두면(전처리), 연산 과정에서 **'소수점 위치 찾기'와 '복잡한 나눗셈'을 제거**할 수 있기 때문입니다.



1. **포맷 전처리**: 실수를 $2^n$배(예: 256배) 하여 정수 포맷으로 통일합니다.
2. **정수 연산**: 하드웨어는 소수점을 신경 쓰지 않고 오직 **정수 곱셈**에만 집중하여 속도를 극대화합니다.
3. **쉬프트 연산 (Shift)**: 고정 소수점 규격의 핵심입니다. 연산 결과로 커진 값을 원래 크기로 돌릴 때, 무거운 나눗셈기 대신 **비트 쉬프트(`>> n`)**를 사용하여 빛의 속도로 복원합니다.



> **결론**: 고정 소수점은 양자화된 데이터를 하드웨어가 **"정수 연산 + 쉬프트 연산"**만으로 처리할 수 있게 만드는 최적의 데이터 포맷입니다. 이를 통해 실제 서비스 속도가 압도적으로 빨라집니다.
